...
public class DriverMatchingService {
    ...
    private void findNearbyDrivers(Booking booking) {
        ExactLocation pickupLocation = booking.getPickupLocation();

        List<Driver> drivers = locationTrackingService.getDriversNearLocation(pickupLocation);
        if(drivers.size() == 0){
        ...
        drivers = filterDriver(drivers, booking);
        //Filter the drivers
    ...
    private List<Driver> filterDriver(List<Driver> drivers, Booking booking) {
    ...
    public static void main(String[] args) {
        //Consumer
        //for each request call appr create method
    }
