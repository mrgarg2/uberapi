...
public class SchedulingService {
    ...
    public void schedule(Booking booking) {
        ...
        //No need to call accept booking
    ...
    public void process(){
        ...
        for (Booking booking: scheduledBookings){
            if(DateUtils.addMinutes(new Date(), constants.getBookingProcessBeforeTime()).after(booking.getSchdeuledTime())){
               //Logical flaw: how driver??
                bookingService.acceptBooking(booking.getDriver(), booking);
            }
            else{
